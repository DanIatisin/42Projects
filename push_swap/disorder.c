...
void	custom_adaptive(t_stack **stack_a, t_stack **stack_b)
{
	...
	disorder = compute_disorder_ps(*stack_a, 0, 0);
	if (disorder < 0.2f)
	{
		simple_sort(stack_a, stack_b);
	}
	//da aggiungere algoritmo n2
	else
	{
		quick_sort_a(stack_a, stack_b, size);
	}
}re un blocco di elementi in due sotto-blocchi usando un valore medio (il pivot):  I numeri più piccoli del pivot vanno in B.  I numeri più grandi o uguali al pivot restano in A.  Poi si fa la stessa cosa ricorsivamente su blocchi sempre più piccoli fino ad arrivare a 1, 2 o 3 elementi.  */
